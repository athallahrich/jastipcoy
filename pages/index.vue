<template>
  <div class="flex flex-col items-center justify-center min-h-[70vh] text-center pt-8">
    ...
    <div class="max-w-2xl mb-12">
      <h1 class="text-6xl font-black text-primary mb-6 font-plus-jakarta tracking-tighter italic">Where to today? 🌸</h1>
      <p class="text-xl text-on-surface-variant font-medium leading-relaxed">Pilih lokasi pengantaran kamu untuk melihat sesi jastip yang tersedia di sekitarmu!</p>
    </div>
    ...
    <div class="w-full max-w-4xl grid grid-cols-2 md:grid-cols-4 gap-6 mb-16 px-4">
      <ClientOnly>
        <button 
    ...
    <div class="w-full max-w-4xl space-y-6 px-4 mb-20">
      <div class="bg-white/50 backdrop-blur-md p-8 rounded-[3rem] border-4 border-dashed border-primary/20 shadow-sm">
        <button 
          class="w-full flex items-center justify-between transition-all group"
          @click="isAddingSpot = !isAddingSpot"
        >
          <div class="flex items-center gap-6">
            <div :class="['w-16 h-16 rounded-[2rem] flex items-center justify-center transition-all duration-500 shadow-lg', isAddingSpot ? 'bg-error text-white rotate-45' : 'bg-primary text-white group-hover:rotate-12']">
              <span class="material-symbols-outlined text-4xl">{{ isAddingSpot ? 'close' : 'add_location' }}</span>
            </div>
            <div class="text-left">
              <h3 class="text-2xl font-black text-on-surface">{{ isAddingSpot ? 'Batal Tambah' : 'Tambah Lokasi Baru' }}</h3>
              <p class="text-on-surface-variant font-medium">Kampus, gym, atau tempat seru lainnya!</p>
            </div>
          </div>
          <span v-if="!isAddingSpot" class="material-symbols-outlined text-primary text-4xl group-hover:translate-x-2 transition-transform">chevron_right</span>
        </button>
      ...
      <ClientOnly>
        ...
        >
          <div class="flex items-center gap-6">
            <div class="w-16 h-16 bg-white/20 rounded-[2rem] flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform">
              <span class="material-symbols-outlined text-4xl fill">{{ user ? 'space_dashboard' : 'auto_awesome' }}</span>
            </div>
            <div class="text-left">
              <h3 class="text-2xl font-black tracking-tight">{{ user ? 'Ke Dashboard Jastiper' : 'Masuk / Daftar Jastiper' }}</h3>
              <p class="text-white/80 font-medium italic">
                {{ user ? 'Pantau orderan dan aktifkan sesi jastipmu! 🚀' : 'Dapatkan penghasilan tambahan sambil jajan! ✨' }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-3 font-black text-sm uppercase tracking-widest bg-white/20 px-6 py-3 rounded-full">
