    ...
    <template v-else-if="order">
      ...
      <div class="flex-1 flex flex-col gap-8">
        ...
        <section class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-primary-container/10 border border-outline-variant/20">
          ...
          <div class="space-y-6">
            <div v-for="(item, idx) in parseItems(order.items)" :key="idx" class="flex gap-6 items-center p-4 rounded-3xl hover:bg-surface-container/30 transition-colors group">
              ...
              <div class="flex-1">
                ...
                <p class="text-on-surface-variant text-sm font-medium">Qty: {{ item.quantity }} x Rp {{ parseInt(item.price).toLocaleString('id-ID') }}</p>
              </div>
          ...
          <div class="mt-8 pt-6 border-t-2 border-surface-container border-dashed flex flex-col gap-3">
            <div class="flex justify-between text-on-surface-variant font-medium">
              <span>Subtotal</span>
              <span>Rp {{ (parseInt(order.amount) - 5000).toLocaleString('id-ID') }}</span>
            </div>
            ...
            <div class="flex justify-between text-primary text-3xl font-black font-plus-jakarta pt-2">
              ...
              <span>Rp {{ parseInt(order.amount).toLocaleString('id-ID') }}</span>
            </div>
          </div>
        </section>
        ...
        <div class="sticky top-24 bg-white rounded-[3rem] p-10 shadow-2xl shadow-primary-container/20 flex flex-col gap-8 border-4 border-primary-container/30 text-center">
          <div v-if="(order.status || 'pending') === 'pending'" class="space-y-6">
            <div class="space-y-2">
              ...
              <p class="text-on-surface-variant font-medium text-sm leading-relaxed">Silakan bayar melalui metode di bawah ini dan kirim bukti ke WhatsApp.</p>
            </div>
            ...
            <div v-if="order.payment_method_id && (order.status === 'pending' || order.payment_receipt)" class="bg-surface-container rounded-[2rem] p-6 border-2 border-primary/5 space-y-4">
            ...
            <div class="flex gap-3">
              <a :href="`https://wa.me/?text=${encodeURIComponent('Halo! Saya ingin konfirmasi pembayaran untuk order #ORDER-' + order.id)}`" target="_blank" class="flex-1 bg-primary text-on-primary font-bold py-4 rounded-full hover:scale-105 transition-all shadow-xl shadow-primary/40 flex justify-center items-center gap-2">
...
<script setup>
const { getOrder, deleteOrder } = useApi();
